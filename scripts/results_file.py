...
class Results(SimpleClass):
    ...
    def plot(
        ...
        if pred_probs is not None  and show_probs:
            text = ",\n".join(f"{names[j] if names else j} {pred_probs.data[j]:.2f}" for j in pred_probs.top5 if pred_probs.data[j]>0.2)
            x = round(self.orig_shape[0] * 0.6)
            y = round(self.orig_shape[0] * 0.02)
            annotator.text([x, y], text, txt_color=(0, 0, 0))  # TODO: allow setting colors
